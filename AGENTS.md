...
> Repository development instructions for **omnixys-debian-13** (Installer).
> Stack: installer. Generated as part of the Omnixys repository-policy rollout.

## Repository Overview

Omnixys Debian 13 installer framework (bash modules + Node tooling, semantic-release, commitlint).

- Repository path: `projects/omnixys-debian-13` (relative to the Omnixys root)
...
core/lib, modules/{ansible, core, custom, docker, k3s, ssh, tailscale}, installer/, plugins/, hooks/, templates/, scripts/, configs/
...
```bash
./build.sh (destructive/root — do not run in this rollout)
```

### Unit tests

```bash
no unit runner defined
```

## Tests

tests/ directory present (framework-specific); no unit runner defined

## Repository-Specific Rules

NOT a Next.js project. Generate installer/bash/Node instructions only. Never expose config.env/identity.env values.
